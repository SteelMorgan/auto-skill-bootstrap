...
def main() -> None:
    ...
    ap.add_argument("--query", action="append", default=[], help="ad-hoc skills search query (repeatable, use with --cap other)")
    ap.add_argument(
        "--ignore-cap",
        action="append",
        default=[],
        help="capability to ignore/decline (repeatable). Ignored caps are treated as resolved and will not be asked again.",
    )
    ...
    ap.add_argument(
        "--non-allowlisted",
        choices=["block", "ignore"],
        default="block",
        help="what to do when missing capabilities only have non-allowlisted candidates: block (exit 2) or ignore (explicit user-decline)",
    )
    args = ap.parse_args()
    ...
    ignored_caps_arg = [c.strip() for c in args.ignore_cap if c and c.strip()]
    if queries and "other" not in caps:
        raise SystemExit("Ad-hoc queries require --cap other. Use: --cap other --query <q1> --query <q2> ...")
    if ("other" in caps) and not queries:
        raise SystemExit("Capability 'other' requires at least one --query.")
    if not caps:
        raise SystemExit("No capabilities provided. Use --cap <capability> (repeatable).")

    # Persist + apply ignored caps (user explicitly declined).
    state_dir = repo_root / ".cursor" / "skills" / "auto-skill-bootstrap"
    state_path = state_dir / "state.json"
    state_existing = read_json(state_path, {})
    ignored_caps_state = state_existing.get("ignored_caps") or []
    ignored_caps = sorted({*(str(x).strip() for x in ignored_caps_state if str(x).strip()), *ignored_caps_arg})
    # Apply ignore only to caps in the current run (avoid global surprises).
    ignored_caps_in_run = sorted({c for c in ignored_caps if c in set(caps)})

    covered = compute_coverage(manifest, caps, capdefs)
    missing = [c for c in caps if (c not in ignored_caps_in_run) and (not covered.get(c))]
    ...
    state["adhoc_queries"] = sorted(set(queries))
    state["ignored_caps"] = ignored_caps

    candidates: Dict[str, Any] = {"generated_at": now, "caps": caps, "missing_caps": missing, "by_cap": {}}

    no_candidates_caps: List[str] = []
    for cap in missing:
        capdef = capdefs.get(cap, {})
        if cap == "other":
            qlist = list(queries)
        else:
            qlist = capdef.get("queries") or [cap]
        ...
        for it in found:
            ...
            dedup.append(it)
        if not dedup:
            no_candidates_caps.append(cap)
        candidates["by_cap"][cap] = {"queries": qlist, "candidates": dedup}

    write_json(candidates_path, candidates)

    # Identify missing capabilities where we found only non-allowlisted candidates.
    non_allowlisted_only_caps: List[str] = []
    for cap, block in (candidates.get("by_cap", {}) or {}).items():
        cand = block.get("candidates", []) or []
        allow = [c for c in cand if c.get("allowlisted")]
        nonallow = [c for c in cand if not c.get("allowlisted")]
        if (cap in missing) and (not allow) and nonallow:
            non_allowlisted_only_caps.append(cap)
    ...
    state["non_allowlisted_only_caps"] = non_allowlisted_only_caps
    state["non_allowlisted_policy"] = args.non_allowlisted
    state["no_candidates_caps"] = no_candidates_caps
    ...
    if no_candidates_caps:
        print("NO SUITABLE SKILLS FOUND for capabilities:")
        for cap in no_candidates_caps:
            print(f"- {cap}")

    # Fail-closed: force the caller (agent) to ask the user before proceeding.
    if non_allowlisted_only_caps and args.non_allowlisted == "block":
        print("USER ACTION REQUIRED: missing capabilities have only non-allowlisted candidates:")
        for cap in non_allowlisted_only_caps:
            print(f"- {cap}")
        sys.exit(2)
